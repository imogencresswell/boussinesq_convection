...
CFL2 = flow_tools.CFL(solver, initial_dt=dt, cadence=1, safety=mag_cfl_safety,
                     max_change=1.5, min_change=0.5, max_dt=max_dt, threshold=0.1)
if threeD:
    CFL2.add_velocities(('Bx*sqrt(Q/Pr)', 'By*sqrt(Q/Pr)', 'Bz*sqrt(Q/Pr)'))
else:
    CFL2.add_velocities(('Bx*sqrt(Q/Pr)', 'Bz*sqrt(Q/Pr)'))
...
try:
    ...
    while (solver.ok and np.isfinite(Re_avg)):
        ...
        if effective_iter % 10 == 0:
            ...
            log_string += 'Time: {:8.3e} ({:8.3e} buoy), step_dt: {:8.3e}, '.format(effective_sim_time, effective_sim_time/t_buoy, step_dt)
