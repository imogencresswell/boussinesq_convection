...
problem.substitutions['f_v_y']   = 'inv_Re_ff*Ky'
problem.substitutions['f_v_z']   = 'inv_Re_ff*Kz'
problem.substitutions['f_i_x']   = 'v*Oz - w*Oy'
problem.substitutions['f_i_y']   = 'w*Ox - u*Oz'
problem.substitutions['f_i_z']   = 'u*Oy - v*Ox'
problem.substitutions['f_ml_x']  = '(M_alfven**-2)*Jy'
problem.substitutions['f_ml_y']  = '(M_alfven**-2)*Jx'
problem.substitutions['f_mn_x']  = '(M_alfven**-2)*(Jy*Bz - Jz*By)'
problem.substitutions['f_mn_y']  = '(M_alfven**-2)*(Jz*Bx - Jx*Bz)'
problem.substitutions['f_mn_z']  = '(M_alfven**-2)*(Jx*By - Jy*Bx)'
problem.substitutions['f_b']     = 'T1'

problem.substitutions['f_v_mag']  = 'sqrt(f_v_x**2 + f_v_y**2 + f_v_z**2)'
problem.substitutions['f_ml_mag'] = 'sqrt(f_ml_x**2 + f_ml_y**2)'
problem.substitutions['f_i_mag']  = 'sqrt(f_i_x**2 + f_i_y**2 + f_i_z**2)'
problem.substitutions['f_mn_mag'] = 'sqrt(f_mn_x**2 + f_mn_y**2 + f_mn_z**2)'
problem.substitutions['f_b_mag']  = 'sqrt(f_b**2)'

problem.substitutions['s_v_mag']  = 'sqrt((f_v_x  - dx(p_v) )**2 + (f_v_y - dy(p_v))**2 + (f_v_z  - dz(p_v) )**2)'
problem.substitutions['s_ml_mag'] = 'sqrt((f_ml_x - dx(p_ml))**2 +   (f_ml_y - dy(p_ml))**2 +  (dz(p_ml))**2)'
problem.substitutions['s_i_mag']  = 'sqrt((f_i_x  - dx(p_i) )**2 + (f_i_y  - dy(p_i) )**2 + (f_i_z  - dz(p_i) )**2)'
problem.substitutions['s_mn_mag'] = 'sqrt((f_mn_x - dx(p_mn))**2 + (f_mn_y - dy(p_mn))**2 + (f_mn_z - dz(p_mn))**2)'
problem.substitutions['s_b_mag']  = 'sqrt(         (dx(p_b) )**2 + (dy(p_b))**2 + (f_b    - dz(p_b) )**2)'
...
problem.substitutions['dxp'] = 'sqrt(dx(p)**2)'
...
analysis_tasks['profiles'].add_task("1 - plane_avg(p)/plane_avg(p_i + p_b + p_v + p_mn + p_ml)", name="p_goodness")
analysis_tasks['profiles'].add_task("plane_avg(sqrt(p_i**2))", name="p_i")
analysis_tasks['profiles'].add_task("plane_avg(sqrt(p_b**2))", name="p_b")
analysis_tasks['profiles'].add_task("plane_avg(sqrt(p_v**2))", name="p_v")
analysis_tasks['profiles'].add_task("plane_avg(sqrt(p_ml**2))", name="p_ml")
analysis_tasks['profiles'].add_task("plane_avg(sqrt(p_mn**2))", name="p_mn")
analysis_tasks['profiles'].add_task("plane_avg(s_v_mag)", name="s_v_mag")
analysis_tasks['profiles'].add_task("plane_avg(s_i_mag)", name="s_i_mag")
analysis_tasks['profiles'].add_task("plane_avg(s_b_mag)", name="s_b_mag")
analysis_tasks['profiles'].add_task("plane_avg(s_mn_mag)", name="s_mn_mag")
analysis_tasks['profiles'].add_task("plane_avg(s_ml_mag)", name="s_ml_mag")
...
CFL2 = flow_tools.CFL(solver, initial_dt=dt, cadence=1, safety=mag_cfl_safety,
                     max_change=1.5, min_change=0.5, max_dt=max_dt, threshold=0.1)
if threeD:
    CFL2.add_velocities(('pi*f*Bx/M_alfven', 'pi*f*By/M_alfven', 'pi*f*Bz/M_alfven'))
else:
    CFL2.add_velocities(('pi*f*Bx/M_alfven', 'pi*f*By/M_alfven', 'pi*f*Bz/M_alfven'))
